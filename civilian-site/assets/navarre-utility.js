...
if(form){
 ...
 fetch('/data/navarre-cost-evidence.json').then(r=>{if(!r.ok)throw Error();return r.json();}).then(d=>{schedule=d;draw();form.querySelector('button').disabled=false;}).catch(()=>{status.textContent='The calculator could not load its reviewed schedule. Use the examples and official rate sheet below.';});
}
