...
// node --env-file-if-exists=.env.local scripts/publish-perdido-guides.mjs --wrangler <existing-cli-path>
...
try{
  for(const c of configs){
    ...
    const exit=await new Promise((resolve,reject)=>{
      const child=spawn(process.execPath,[wrangler,'pages','deploy',join(candidate.candidate,c.site),'--project-name',c.name,'--branch','main','--commit-dirty=true','--commit-message','Reviewed Perdido Key coastal and PCS community guides'],{
