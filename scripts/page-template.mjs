// Page template used by build-new-pages.mjs to generate HTML pages that match
// the existing public/*.html visual and schema structure.

const SAME_AS = JSON.stringify([
...
const PERSON_SCHEMA = `{"@context":"https://schema.org","@type":"Person","@id":"https://pensacolamilitaryhousing.com/#person-gregg","name":"Gregg Costin","alternateName":"Gregg Costin, Realtor","givenName":"Gregg","familyName":"Costin","jobTitle":"Realtor, Military Relocation Specialist","worksFor":{"@type":"RealEstateOrganization","name":"Levin Rinke Realty","url":"https://greggc.levinrinkerealty.com","address":{"@type":"PostalAddress","streetAddress":"220 W. Garden Street","addressLocality":"Pensacola","addressRegion":"FL","postalCode":"32502","addressCountry":"US"}},"alumniOf":[{"@type":"Organization","name":"United States Air Force"},{"@type":"CollegeOrUniversity","name":"University of Tampa"}],"description":"Retired USAF Captain and E-3 AWACS Combat Systems Officer. Completed a full military career from prior-enlisted Staff Sergeant to commissioned officer with 11 PCS moves. Now a licensed Florida and Alabama Realtor specializing in military relocation, VA loans, and PCS moves across the Florida Panhandle.","email":"Gregg.Costin@gmail.com","telephone":"+1-850-266-5005","url":"https://pensacolamilitaryhousing.com","image":"https://pensacolamilitaryhousing.com/images/gregg-portrait.jpg","sameAs":${SAME_AS}}`;

const GA4_SCRIPT = `<script async src="https://www.googletagmanager.com/gtag/js?id=G-W29GHBK38M"></script>
<script>
...
.main-banner{background:var(--panel);border-bottom:1px solid var(--hair);position:sticky;top:0;z-index:50}
.banner-row{max-width:1320px;margin:0 auto;padding:10px 16px;display:grid;grid-template-columns:auto auto 1fr;align-items:center;gap:16px}
...
.dropdown-menu{display:none;position:absolute;top:100%;left:0;background:var(--panel);border:1px solid var(--hair);min-width:260px;z-index:1000;box-shadow:0 10px 30px rgba(0,0,0,.4)}
.dropdown:hover .dropdown-menu,.dropdown:focus-within .dropdown-menu{display:block}
.dropdown-menu a{display:block;padding:10px 14px;color:var(--muted);font-size:12px;border-bottom:1px solid var(--hair);text-transform:none;letter-spacing:.3px;font-weight:400}
...
<div class="related">
<a href="/va-loan-pensacola.html">VA Loan Pensacola</a>
...
<a href="/va-disability-property-tax-florida.html">VA Disability Property Tax</a>
...
MRP &middot; ABR &middot; SRS &middot; RENE &middot; Florida Military Specialist &middot; Pursuing GRI</p>
...
<p data-last-updated="true" style="text-align:center;font-size:.75rem;color:var(--mutedD,#6B7280);margin:.5rem auto 0;letter-spacing:.5px;text-transform:uppercase">Last updated: April 22, 2026</p>
</footer>`;

const CTA = `<div class="cta">
<p style="margin:0 0 .5rem"><strong>Ready to make your move with a military-insider Realtor?</strong></p>
<p style="margin:0">Call or text <a href="tel:+18502665005">(850) 266-5005</a> &nbsp;|&nbsp; Email <a href="mailto:Gregg.Costin@gmail.com" style="color:#fff">Gregg.Costin@gmail.com</a></p>
</div>`;
...
function agentSchema(area, knows) {
  return JSON.stringify({
    ...
    sameAs: JSON.parse(SAME_AS),
  });
}

export function renderPage(p) {
  const canonical = `https://pensacolamilitaryhousing.com/${p.slug}.html`;
  const faqJson = p.faqs && p.faqs.length ? `<script type="application/ld+json">\n${faqSchema(p.faqs)}\n</script>` : "";
  const placeJson = p.place ? `<script type="application/ld+json">\n${JSON.stringify(p.place)}\n</script>` : "";
  const agentJson = `<script type="application/ld+json">\n${agentSchema(p.areaServed || p.h1, p.knowsAbout || [])}\n</script>`;
  const breadJson = `<script type="application/ld+json">\n${breadcrumbSchema(p.slug, p.breadcrumbName || p.h1)}\n</script>`;
  const personJson = `<script type="application/ld+json">\n${PERSON_SCHEMA}\n</script>`;

  const factsHtml = p.facts ? `<div class="facts">\n${p.facts.map(([k, v]) => `<div><strong>${k}:</strong> ${v}</div>`).join("\n")}\n</div>` : "";
...
<meta name="keywords" content="${escapeAttr(p.keywords || "")}">
...
${TOPNAV}
<header>
<nav class="crumbs"><a href="/">Home</a> &rsaquo; <a href="/#resources">Resources</a> &rsaquo; ${p.breadcrumbName || p.h1}</nav>
<h1>${p.h1}</h1>
