// Apply the SPA banner styling uniformly to every static HTML page.
// Handles two existing patterns:
//   A. "old uniform" pattern (~22 pages): replace the whole banner block
//   B. base/community/faq/reviews/blog pattern (~26 pages): they already have
//      sticky+blur but tab styling differs — normalize tab rules only
...
// ── Pattern A: old uniform banner block → replace entirely ─────────────────
const OLD_A = `.main-banner{background:var(--panel);border-bottom:1px solid var(--hair)}
...
const NEW_A = `.main-banner{background:rgba(10,15,26,0.95);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);border-bottom:1px solid var(--hair);position:sticky;top:0;z-index:1000;transition:all .3s ease}
...
// ── Pattern B: base/community pages — tab rule tweaks to match SPA pill style
const OLD_B_TAB = ".main-banner .banner-tabs>a,.main-banner .banner-tabs .dropdown>button{background:transparent;border:none;color:#A5A496;padding:8px 14px;font-size:11px;font-weight:600;letter-spacing:1.5px;text-transform:uppercase;cursor:pointer;font-family:inherit;text-decoration:none;transition:color .2s}";
const NEW_B_TAB = ".main-banner .banner-tabs>a,.main-banner .banner-tabs .dropdown>button{background:transparent;border:none;color:rgba(255,255,255,0.8);padding:6px 10px;font-size:11px;font-weight:500;letter-spacing:.5px;text-transform:uppercase;cursor:pointer;font-family:inherit;text-decoration:none;border-radius:4px;transition:all .2s}";

const OLD_B_HOVER = ".main-banner .banner-tabs>a:hover,.main-banner .banner-tabs .dropdown>button:hover{color:#C9A84C}";
const NEW_B_HOVER = ".main-banner .banner-tabs>a:hover,.main-banner .banner-tabs .dropdown>button:hover{background:rgba(201,168,76,0.15);color:#C9A84C}";

const OLD_B_EMAIL = ".main-banner .banner-email{color:#C9A84C;font-size:14px;font-weight:600;text-decoration:none;letter-spacing:.3px;white-space:nowrap}";
const NEW_B_EMAIL = ".main-banner .banner-email{color:#C9A84C;font-size:14px;font-weight:600;text-decoration:none;letter-spacing:.3px;white-space:nowrap;text-transform:none}";

const files = readdirSync(PUB).filter(f => f.endsWith(".html"));
let counts = { A: 0, B: 0, skipped: 0 };
...
for (const f of files) {
  ...
  if (html.includes(OLD_A)) {
    html = html.replace(OLD_A, NEW_A);
    counts.A++;
  } else if (html.includes(OLD_B_TAB)) {
    html = html.replace(OLD_B_TAB, NEW_B_TAB);
    if (html.includes(OLD_B_HOVER)) html = html.replace(OLD_B_HOVER, NEW_B_HOVER);
    if (html.includes(OLD_B_EMAIL)) html = html.replace(OLD_B_EMAIL, NEW_B_EMAIL);
    counts.B++;
  } else {
    counts.skipped++;
    continue;
...
console.log(`Pattern A (full banner replace): ${counts.A} pages`);
console.log(`Pattern B (tab style tweak):     ${counts.B} pages`);
console.log(`Skipped (already matched or no banner): ${counts.skipped}`);
