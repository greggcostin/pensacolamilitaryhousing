...
import { REMAINING_GUIDES } from "../content/geo/remaining-financial-guides.mjs";
...
export const QUICK_ANSWERS = {
  ...
  "public/bah-to-mortgage-guide.html": GUIDES['bah-to-mortgage-guide'].quickAnswer,
  "public/va-loan-guide.html": GUIDES['va-loan-guide'].quickAnswer,
...
for (const [slug, spec] of Object.entries({...FINANCIAL_GUIDES,...REMAINING_GUIDES})) QUICK_ANSWERS[`public/${slug}.html`] = spec.quickAnswer;
...
for (const [file, text] of Object.entries(QUICK_ANSWERS)) {
  ...
  if (/data-(?:zip-study|regional-guide|cited-review)=|data-accuracy-review="bases\//.test(html)) continue; // Dedicated reviewed renderer owns this answer and its date.
  const core = GUIDES[slug] || FINANCIAL_GUIDES[slug] || REMAINING_GUIDES[slug];
  const date = core ? new Date(REVIEWED + 'T12:00:00Z').toLocaleDateString('en-US', {month:'long',day:'numeric',year:'numeric',timeZone:'UTC'}) : DATE;
  const out = placeQuickAnswer(html, { text, date, by: file.startsWith("civilian-site") ? GC_BY : undefined });
