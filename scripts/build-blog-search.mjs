...
for(const key of selected){const site=SITES[key];if(!site)throw Error('Unknown site '+key);
 ...
 if(process.argv.includes('--preserve-photography')){
  const findings=photographyFindings(root);
  if(findings.length)throw Error('Existing photography must pass before indexing without regeneration: '+JSON.stringify(findings));
 }else{
