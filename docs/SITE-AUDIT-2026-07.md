...
**Still open (larger / higher-regression-risk, deferred pending go-ahead):** 2.14 analytics rewiring (SPA `page_view` in `go()`, findbuyers/calendly/825bayshore click events, calculator/FAQ events); 2.15 SPA architecture (per-route prerender shells for `/pcs-guide` `/mortgage-calculators` `/about` `/contact`, blog pipeline decision, `hydrateRoot`, fonts to `<link>`); 2.16 remainder (form `htmlFor`/`id`, dropdown ARIA + touch, skip link, modal focus trap); 2.17 one build-time data file for perishable figures; 2.18 internal-link graph trim + contextual links + breadcrumb fragment fix + llms.txt regen. Also NOT done by choice: the homepage self-serving aggregateRating/Review block (real named reviews; Google won't show self-serving stars and it's manual-action-adjacent — left for Gregg's call since it's the homepage).
