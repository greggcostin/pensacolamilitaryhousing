...
if(!/^[a-f0-9]{40}$/.test(c.sourceCommit||''))throw Error('Verified source commit required');
...
try{
  ...
  for(const s of sites){
    ...
    const exit=await new Promise((resolve,reject)=>{const child=spawn(process.execPath,[wrangler,'pages','deploy',join(c.candidate,s.site),'--project-name',s.project,'--branch','main','--commit-dirty=true','--commit-hash',c.sourceCommit,'--commit-message',c.releaseMessage||'Source-verified Costin website refinement'],{env:{...process.env,CLOUDFLARE_API_TOKEN:token,CLOUDFLARE_ACCOUNT_ID:account,CI:'true'},stdio:'inherit',windowsHide:true});child.on('error',reject);child.on('exit',resolve);});
