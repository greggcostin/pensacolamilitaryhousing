...
The latest approved photography treatment, homepage credentials and review proof, responsive hero preload, and direct Google/Zillow review links now live in the same maintained source as the school, blog and GEO work. The integration starts from GitHub main `55704ee99aa4a6e2ec8f24e38767334e30761e76` and preserves the newer source safeguards instead of replacing them with the older shared working checkout.

The production reference was verified on September 9, 2026 against all 4,981 Cloudflare asset hashes: military `e912159d-0e14-497d-a5eb-5264623575c6` and civilian `85dd2370-90c8-4548-a677-cd5d0aca3967`. These are the photography release and its subsequent homepage/review refinements. The implementations came from **Audit and elevate greggcostin.com**, task `01a074bb-41ca-7342-a229-cbf774e1dfa5`. The preceding [catalog source handoff](../blog-catalog-refresh-2026-09-09.md), [school/source integration](2026-09-09-cross-site-integration.md) and [homepage preload](2026-09-09-homepage-image-preload.md) remain part of this source.
...
- `npm run build:civilian`: checks civilian blog source, finalizes photography and delivery, rebuilds the complete English search index and runs the civilian audit.
...
Stylesheet restoration and rebuilding no longer accumulate blank lines. The complete civilian homepage pipeline was run again across 1,591 local delivery files with zero changed files, zero new files and unchanged review observations. Existing analytics guards and deferred receipt loading were retained, including restoring missing generated-page guards. The existing Whiting guide link remains; a contextual link from the Crestview community page restores discovery of the existing military relocation guide.

## Verification

| Check | Result |
| --- | --- |
| Approved production baseline | All 4,981 provider hashes matched before integration |
...
| Photography/homepage browser checks | 29 passed, including phone/desktop review proof, links and no-JavaScript footer |
...
This is source reconciliation, not a new production deployment. The approved design was already live; the source-only generator, validation, homepage-data and discovery corrections require a later verified release to become live. Military automatic Git deployment remains disabled, and civilian publication is separate. Before a later publication, refresh both current production identities and compare the complete built candidate with the latest complete release. Do not deploy the older dirty shared checkout or mistake a source push for publication, indexing, recommendation or referral evidence.
