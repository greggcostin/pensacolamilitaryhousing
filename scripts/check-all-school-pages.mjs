...
check('Private, Alabama, missing-grade and virtual records never acquire invented scores or campus pins',()=>{
 for(const s of records){if(legacy[s.reportUrl])continue;const h=html(s.reportUrl);if(s.sector==='private')assert(h.includes('There is no public-school accountability letter'));if(s.state==='AL'&&s.sector==='public'){assert(h.includes('Alabama')&&h.includes('2024–25'));assert(h.includes('sg-alabama-result'));if(s.grade)assert(h.includes(s.alabamaAccountability.score+'/100 total points'));else assert(/Approved Waiver|No school-level result/.test(h));}if(s.virtual){const school=[...h.matchAll(/<script type="application\/ld\+json"[^>]*>([\s\S]*?)<\/script>/g)].map(m=>JSON.parse(m[1])).find(x=>x['@type']==='School');assert(!school.geo);}}
});
