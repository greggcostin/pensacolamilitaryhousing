...
import { REVIEW_COUNT_PATTERNS, syncReviewText, validateCount,reviewCheckDate,reviewSourceNote } from './review-counts-lib.mjs';
...
const observed={google:{checkedAt:'2026-09-09T04:30:33.984Z'},zillow:{checkedAt:'2026-09-10T04:30:33.984Z'}};
assert.equal(reviewCheckDate(observed),observed.google.checkedAt);
assert.ok(reviewSourceNote(reviewCheckDate(observed)).includes('September 8, 2026 (Central Time)'));
const oldNote='<p class="gc-review-source-note">Counts recorded September 6, 2026: Google confirmed by Gregg.</p>';
const nextNote=syncReviewText(oldNote,counts,reviewCheckDate(observed));
assert.ok(!nextNote.includes('confirmed by Gregg'));assert.ok(nextNote.includes('September 8, 2026'));
assert.equal(syncReviewText(nextNote,counts,reviewCheckDate(observed)),nextNote);
assert.equal(syncReviewText(oldNote,counts),oldNote);
assert.throws(()=>reviewCheckDate({google:{checkedAt:'bad'}}));
