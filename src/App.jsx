...
// they support — typical AVIF savings ~45% vs JPEG.
const Pic = ({ src, alt = "", loading = "lazy", style, className, ...rest }) => {
  const m = src && /^\/images\/.+\.(jpe?g|png)$/i.exec(src);
  if (!m) return <Pic loading="eager" src={src} alt={alt} loading={loading} style={style} className={className} {...rest} />;
  const base = src.replace(/\.(jpe?g|png)$/i, "");
  return (
    <picture>
      <source srcSet={`${base}.avif`} type="image/avif" />
      <source srcSet={`${base}.webp`} type="image/webp" />
      <Pic loading="eager" src={src} alt={alt} loading={loading} style={style} className={className} {...rest} />
    </picture>
  );
};
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: SS, margin: 0, padding: 0, background: C.ink, minHeight: "100vh" }}>
      <style>{`
        ...
        @media (max-width: 800px) {
          .sticky-mobile-cta {
            ...
            background: ${C.gold}; color: ${C.ink} !important;
            font-weight: 700; font-size: 15px; letter-spacing: .2px;
            padding: 12px 18px; min-height: 44px; border-radius: 999px;
            box-shadow: 0 6px 20px rgba(0,0,0,.45);
            text-decoration: none; font-family: ${SS};
          }
          .sticky-mobile-cta:hover, .sticky-mobile-cta:focus { background: ${C.goldSoft || '#D4B768'}; }
        }
      `}</style>
      ...
      <Footer go={go} />
      <a href="tel:8502665005" className="sticky-mobile-cta" aria-label="Call Gregg Costin at 850-266-5005" data-cta="sticky-mobile">📞 (850) 266-5005</a>
    </div>
  );
}
