...
if(!assessment.changed){save(join(dir,'result.json'),{checkedAt:new Date().toISOString(),status:'unchanged',counts:assessment.counts});console.log('Counts unchanged; no publication needed.');process.exit(0);}
...
for(const s of proof.sites)for(const f of walk(join(candidate,s.site)).filter(f=>/\.(?:html|txt|js|json)$/.test(f)&&!f.replaceAll('\\','/').includes('/pagefind/'))){const old=readFileSync(f,'utf8'),next=syncReviewText(old,assessment.counts);if(old!==next)writeFileSync(f,next);}
