...
const verifiedAt=reviewCheckDate(c.observation);
...
for(const s of c.production){const root=join(c.candidate,s.site);
 for(const f of walk(s.localBaseline)){const path=relative(s.localBaseline,f).replaceAll('\\','/'),target=join(root,path);if(!existsSync(target)){if(s.site==='pmh'&&path.startsWith('pagefind/'))continue;issues.push('Removed '+s.site+'/'+path);continue;}
  ...
  if(/\.(?:html|txt|js|json)$/.test(path)){const old=readFileSync(f,'utf8'),next=readFileSync(target,'utf8');if(next!==syncReviewText(old,c.counts,verifiedAt))issues.push('Non-review change '+s.site+'/'+path);if(syncReviewText(next,c.counts,verifiedAt)!==next)issues.push('Stale count or date '+s.site+'/'+path);}
