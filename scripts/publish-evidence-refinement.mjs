// Publish only the sealed community guide release, and only if neither production baseline moved.
// node --env-file-if-exists=.env.local scripts/publish-evidence-refinement.mjs --wrangler <existing-cli-path>
import {readFileSync,writeFileSync,readdirSync,existsSync} from 'node:fs';
import {join,relative} from 'node:path';
import {createHash} from 'node:crypto';
import {spawn} from 'node:child_process';
const directory=process.argv.includes('--evidence-dir')?process.argv[process.argv.indexOf('--evidence-dir')+1]:'docs/seo-geo-2026-09-06/projects/03-accuracy/zip-and-claims';
const candidate=JSON.parse(readFileSync(join(directory,'candidate-manifest.json'),'utf8'));
const gates=JSON.parse(readFileSync(join(directory,'candidate-verification.json'),'utf8'));
...
try{
  for(const c of configs){
    if(!candidate.changes.some(change=>change.site===c.site))continue;
    ...
    const exit=await new Promise((resolve,reject)=>{
      const child=spawn(process.execPath,[wrangler,'pages','deploy',join(candidate.candidate,c.site),'--project-name',c.name,'--branch','main','--commit-dirty=true','--commit-message','Reviewed ZIP study, financial claims, receipt conversions and Navarre guides'],{
