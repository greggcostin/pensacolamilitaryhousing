...
const block=`${start}\n## Complete school guides\n\n${ctx.directory.schools.length} source records map to ${schools.length} canonical school guides. Duplicate federal identities share a school page. Private-school guides describe published educational approaches and admissions; private schools have no Florida public-school accountability grade. Directory years and verified campus updates are identified per page.\n\n${schools.map(s=>`- [${s.name}](https://greggcostin.com${s.reportUrl}): ${schoolFormat(s)}, ${s.city}, ${s.state}; school data, local planning context, enrollment and comparison resources.`).join('\n')}\n${end}`;
