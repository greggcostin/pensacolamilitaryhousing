...
export function readResearch(slug, site = 'gc', root = ROOT) {
  if (!SITES[site]) throw new Error('Unknown editorial site: ' + site);
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug || '')) return null;
  const path = `${root.replace(/\\/g, "/").replace(/\/$/, "")}/${SITES[site].contentDir}/research/${slug}.json`;
...
export function voiceFindings(spec, body) {
  const text = JSON.stringify([spec.title, spec.description, spec.h1, spec.lead, spec.excerpt, spec.category, spec.quickAnswer, spec.takeaways, spec.shareHook, spec.faqs || spec.faq, spec.related?.map(r => r.label), spec.figure?.alt, spec.figure?.caption]) + String(body).replace(/data-inquiry-type=["'][^"']*["']/g, '');
...
export function validateEditorial(spec, body, research, { today = new Date().toISOString().slice(0, 10), requireReview = true, site = 'gc' } = {}) {
  const errors = voiceFindings(spec, body), warnings = [];
  const faqs = spec.faqs || spec.faq || [];
  const combined = normalize([spec.lead, spec.quickAnswer, ...(spec.takeaways || []), body, ...faqs.map(f => f.q + ' ' + f.a)].join(' '));
  ...
  if (faqs.length < 6) errors.push('FAQ: requires at least 6 distinct questions');
  if (new Set(faqs.map(f => normalize(f.q).toLowerCase())).size !== faqs.length) errors.push('FAQ: repeated question');
  if (faqs.some(f => words(f.a) < 40 || words(f.a) > 95)) errors.push('FAQ: each answer must contain 40-95 useful words');
  ...
  if (research.templateOnly) errors.push('research: a template is not publishable evidence');
  ...
  if (requireReview) {
    ...
    if (review?.checkedAt < research.sessionDate || sources.some(s => review?.checkedAt < s.checkedAt)) errors.push('review: final review predates source verification');
  }
