...
const root=path.join(import.meta.dirname,'preview'),base=path.resolve(import.meta.dirname,'../2026-09-12-autopilot/release/gc'),source=path.join(import.meta.dirname,'source'),report=path.join(source,'docs/civilian-blog-engine/2026-09-14');
...
const inbound=['buy','sell','resources/mortgage-preapproval','blog/fed-rate-hike-what-it-means'].map(rel=>({hub:rel,url:'https://greggcostin.com/'+rel,exists:fs.readFileSync(path.join(root,rel+'.html'),'utf8').includes('href="/blog/'+slug+'"'),unchanged:sha(path.join(root,rel+'.html'))===sha(path.join(base,rel+'.html'))}));
